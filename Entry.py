...
def SubmitEntry(entries1,entries2,cr_fields,de_fields,fields,row,username,row1,row2,cfont,b1):
    ...
    for c in cr_fields:
        if c=="Cr_Date":
            cr_fields_list.append(str(entries1['Date'].get_date()))
        elif c!="Cr_Total":
            cr_fields_list.append(str(entries1[c].get()))
    
    for d in de_fields:
        if d=="Deb_Date":
            de_fields_list.append(str(entries1['Date'].get_date()))
        elif d!="Deb_Total":
    ...
    #Updating the Credit Table with New Data.
    CreateTable.CreditEntry(cr_fields_list,fields,username)  
    #Updating the Debit Table with New Data.
    CreateTable.DebitEntry(de_fields_list,fields,username)
    
    #Collecting the Balance for Display...
    data,cr,de=CreateTable.CheckBalance(username,str(entries1['Date'].get_date()))
...
def Cr_SubmitEntry(entries1,cr_fields,fields,row,username,row1,cfont,b1):
    ...
    data,cr,deb=CreateTable.CheckBalance(username,str(entries1['Date'].get_date()))
...
def De_SubmitEntry(entries2,de_fields,fields,row,username,row2,cfont,b1):
    ...
    for d in de_fields:
        if d=="Deb_Date":
            de_fields_list.append(str(entries2['Date'].get_date()))
        elif d!="Deb_Total":
            de_fields_list.append(str(entries2[d].get()))
    ...
    data,cr,deb=CreateTable.CheckBalance(username,str(entries2['Date'].get_date()))
...
class Entrys:
    def AllEntrytLoop(username):
        ...
        #For Label and Title
        Commonscreen.Commonscreen(credit,"Credit/Debit Entry Table",'Credit/Debit Entry Table')
        
        #Collecting the Fields(Bank Accounts)
        ...
        #Button for Submiting the Data which was given by the User..    
    ...
    def CreditEntry(username):
        ...
        Commonscreen.Commonscreen(credit,"Credit Entry Table",'Credit Entry Table')
    ...
    def DebitEntry(username):
        ...
        Commonscreen.Commonscreen(credit,"Debit Entry Table",'Credit/Debit Entry Table')
